...
func TestSeedCheckAutoStar(t *testing.T) {
	...
	g.NextEvent.Handle(PBCSeedCheckBowser, &g) //Picked Bowser
...
func TestSeedCheckAutoBowser(t *testing.T) {
	...
	g.NextEvent.Handle(2, &g)                //Move
	g.NextEvent.Handle(PBCSeedCheckToad, &g) //Picked Toad
	g.NextEvent.Handle(2, &g)                //Move
	g.NextEvent.Handle(PBCSeedCheckToad, &g) //Picked Toad
	g.NextEvent.Handle(2, &g)                //Move
	g.NextEvent.Handle(PBCSeedCheckToad, &g) //Picked Toad
...
func TestPiranha(t *testing.T) {
	...
	g.NextEvent.Handle(PBCPiranhaDecisionPay, &g) //Pay 30 coins
